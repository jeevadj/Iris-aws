                ...
                cameraSource.takePicture(null, new CameraSource.PictureCallback() {
                    ...
                    public void onPictureTaken(byte[] bytes) {
                        ...
                        if(!tvvs.isSpeaking()){
                            HashMap<String,String> params=new HashMap<String, String>();
                            params.put(TextToSpeech.Engine.KEY_PARAM_UTTERANCE_ID,"sampleText");
                            tvvs.speak("picture captured please wait few seconds for a results ",TextToSpeech.QUEUE_ADD,params);
                        }
...
//                        }
                        ...
                        Toast.makeText(textscanner.this, path, Toast.LENGTH_LONG).show();
                        //actualImage= getExternalFilesDir(path).getAbsoluteFile();
                        try {
                            actualImage = FileUtil.from(getApplicationContext(),Uri.fromFile(getExternalFilesDir(path).getAbsoluteFile()));
                        } catch (IOException e) {
                            e.printStackTrace();
                        }
                        System.out.println("Bowwwwww"+actualImage.getAbsoluteFile());
                            try {
                                compressedImage = new id.zelory.compressor.Compressor(getApplicationContext())
                                        .setMaxWidth(500)
                                        .setMaxHeight(500)
                                        .setQuality(75)
                        ...
                        File file = new File(actualImage.getAbsolutePath());
                        byte[] b = new byte[(int) file.length()];
                        image = new Image();
                        image.withBytes(ByteBuffer.wrap(b));
...
//                }
    ...
    public class Rekog_Text_Task extends AsyncTask<String, Integer, String >{
        ...
        protected String doInBackground(String... strings) {
            ...
            stringBuilder = new StringBuilder();
            for(int i=0; i<(int)bow.size()/2;i++){
                if(bow.get(i).getConfidence()>75){
                ...
                public void run() {
                   ...
                   // speakout(stringBuilder.toString());
                    if(!tvvs.isSpeaking()){
                        HashMap<String,String> params=new HashMap<String, String>();
                        params.put(TextToSpeech.Engine.KEY_PARAM_UTTERANCE_ID,"sampleText");
                        tvvs.speak(stringBuilder.toString().toLowerCase(),TextToSpeech.QUEUE_ADD,params);
                    }
